		...
		.end(function (res) {
			if (res.error) {
				if (res.body.sessError) {
					...
					if (res.body.inputError[0].msg == 'Invalid Category containing Products'){
						if (confirm('You want to delete all the products in this category?') == true) {
							superagent
								.post('/admin/api/cat/removeAll')
								.send(serialize)
								.end(function (res) {
									if (res.error) {
										if (res.body.sessError) {
...
function onChangeHandler(e) {
	if (this.value != '') {
		superagent
			.post("/admin/api/prod/edit/change")
			...
			.end(function (res) {
				...
				if (res.body.prodEdit){
					var cat = document.getElementById('prodEditCatId');
					for (var i = 0; i <= cat.options.length - 1; i++) {
						if (res.body.prodEdit[0].catid == cat.options[i].value) {
							cat.options[i].selected = 'selected';
							break;
						}
					}
					document.getElementById('prodEditName').value = res.body.prodEdit[0].name;
					document.getElementById('prodEditPrice').value = res.body.prodEdit[0].price;
					document.getElementById('prodEditDescription').innerHTML = res.body.prodEdit[0].description;
					var blankImage = document.createElement("img");
					blankImage.id = 'prodOrgImage';
					document.getElementById('prodEditImageDiv').appendChild(blankImage);
					document.getElementById('prodOrgImage').src = res.body.prodEdit[0].imageURL;
				}	
			});	
	} else {
...
document.querySelector('#productNewPanel form').addEventListener('submit', onSubmitImageHandler);
document.querySelector('#productEditPanel form').addEventListener('submit', onSubmitImageHandler);
document.querySelector('#productRemovePanel form').addEventListener('submit', onSubmitHandler);

document.querySelector('#prodEditPId').addEventListener('change', onChangeHandler);
